...
> **訂正（2026-09-07）：** 国立環境研究所福島支部にメールで確認したところ、本論文で
> 紹介されているCHPは**実際には導入されておらず**、仮想的な条件を設定して計算した
> シミュレーションであることが判明しました。年間700〜800トンや≤50kWといった数値は
> **計画上のベンチマーク**として引き続き有用ですが、以下の記述でCHPが実際に稼働・
> 検証済みであるかのように読める箇所は、論文のシミュレーション上のシナリオを指す
> ものと読み替えてください。薪ステーション事業・物産館ボイラー・HEMS調査・協議会
> 体制など、CHP以外の要素は町の実際の取り組みである可能性がありますが、真偽は
> 再確認していません。
...
本研究は、福島県大沼郡三島町（奥会津地域）という小さな過疎山間地において、**劣化した森林問題**と**外部エネルギーへの依存**という二つの危機を、**小規模な木質バイオマス熱電併給（CHP）システム**によって同時に解決しようとした取り組みを検証したものです。研究者（国立環境研究所）は三島町と連携し、地域の森林資源のみを活用したエネルギーシステムのシミュレーション設計に取り組みました（実機の導入ではなく、上記訂正を参照）。
...
| アクション | 論文に基づく根拠 |
|---|---|
| 小型CHP（≤50kW）の導入 | 三島町のシミュレーション上の可能性調査に基づく計画基準値（実証済みではない） |
| 流通半径内でのバイオマス地産地消 | 原料輸送上限50〜125kmが確認済み；御杖村の自村森林は範囲内 |
| パイロット建物へのHEMS設置 | 三島町のシミュレーションではモデル化された需要を使用；御杖村では独自のHEMS調査を実施してからデータセンター負荷仕様を確定する |
| FIT登録（売電） | 三島町のシミュレーション上の採算モデルはFITに依拠 — 制度変更前の早期登録を推奨 |
...
1. **年間700〜800トンのバイオマスで50kW以下のCHP** が御杖村パイロットの適切な規模 — 同条件の村落のシミュレーションに基づく計画基準値（国立環境研究所が2026-09-07にCHP未導入・シミュレーションであることを確認済み、実データによる検証ではない）。
...
7. **御杖村は一人ではない** — 三島町は直接比較可能なシミュレーション研究事例；補助金申請や村行政との協議で「厳密な計画事例」として引用するが、「他所で実現済み」とは表現しない。
