...
<p style="font-style:italic; color:#666; margin:2mm 0 8mm;">バイオマスエネルギーとAI — アーンドバリュー管理計画</p>
...
本計画の対象期間は**2026年4月〜2028年9月（30か月間）**です。フェーズ4（運営・拡大期、31か月目以降）は資金調達方法が異なり（事業収益による）、フェーズ1の結果確認後に翌年計画するため、基準値には含みません。*フェーズ4を支える事業収益モデルおよび資本回収分析は `mitsue_revenue_model_jp.md` および実施計画の拡充されたフェーズ4セクションに記載しています。本PMBは意図的にフェーズ3（建設）終了時点で止めています。この支出ベースラインに対し資金残高を常にプラスに保つための月次の資金**流入**は `mitsue_cashflow_model_jp.md` に記載しています。*
...
### 8.1 流動性管理 — 資金残高は絶対ルール

SPI/CPIは*効率*を測るものであり、手元資金の有無は示しません。流入がまとまった塊で入る一方で支出は連続的に発生するため（`mitsue_cashflow_model_jp.md` 参照）、本プロジェクトは別途、**月次資金残高＝累積流入−累積流出**を必須の下限（フロア）に照らして管理します：

| 流動性フロア | ルール |
|---|---|
| **フェーズ0〜2** | 残高は**¥5M以上**を維持（M7の谷では絶対最低¥3M；この時期は創業者資本が唯一の流入） |
| **フェーズ3** | 残高は**月次バーンの1か月分以上**を維持（M25〜M26のピーク時は¥25M以上） |
| **抵触予測** | 3か月ローリングの残高予測がフロア抵触を示した場合、代表理事は抵触月の**前に**トランシェの前倒しまたはブリッジ資金の引出しを発動 |

これらのフロアは**目標ではなく絶対ルール**です：抵触予測はレッドのSPI/CPIと同等の停止・対応条件です。月次報告は**支出だけでなく残高**を追跡します（§9）。
...
| 頻度 | 報告書 | 宛先 | 内容 |
|------|--------|------|------|
| **月次** | EVM状況報告 | 理事会・主要資金提供者 | PV/EV/AC表；SPI/CPI；SV/CV；EAC更新；**§8.1流動性フロアに対する資金残高＋3か月残高予測**；課題 |
...
フェーズ1以降、月次報告は毎回、§8.1のフロアに照らした資金残高ライン（累積流入−累積流出）を含めます——本プロジェクトは**支出だけでなく残高**を追跡します。
...
- **§8.1の流動性フロアは拘束的：** 月次資金残高はフロアを下回ってはならず、各トランシェはそれが賄う支出より前に着金するよう計画すること
- **フェーズ3の資金トランシェは、計画着金月の2〜3か月前に契約締結すること** — 補助金の後払い（精算払い）ラグを吸収するため
- **約¥25Mのブリッジ枠**（銀行融資枠、マネジメント予備費の取崩し、または契約済み補助金に対する前渡し）を、M22〜M28のバーンピークに対する運転資金のショックアブソーバーとしてゲート3までに用意すること
...
- **Rev.2**（計画 M9、2026年12月）— フェーズ1完了後の改訂。フィージビリティスタディ結果とベンダー見積に基づく。**確定した地域脱炭素移行・再エネ推進交付金額も折り込む予定**（太陽光・蓄電池・EV・自営線設備費の補助率2/3〜3/4、村の官民連携経由）——フェーズ2〜3で¥28M〜¥53Mの資金ギャップを埋める最も具体的な道筋であり、これは**ゲート3（M21）までに確保・支出可能とすること**が必須で、事業終了時まで先送りしてはならない（`mitsue_cashflow_model_jp.md` §5参照）。Rev.2ではさらに、コスト再同期と歩調を合わせて**キャッシュフローモデルを稼働中のガントへ再タイミング**し、確定した交付金の支出時期およびブリッジ融資条件を折り込むこと。出典：https://policies.env.go.jp/policy/roadmap/grants/ · https://www.env.go.jp/content/900470616.pdf
