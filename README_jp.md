<p align="right">バージョン: v2.1 &nbsp;|&nbsp; 最終更新: 2026-05-23</p>
...
御杖プロジェクトは、閉校となった御杖小学校とその周辺の森林景観を、農村再生の統合的なモデルとして再活用するための非営利イニシアチブです。本プロジェクトは、在来種の森林再生、個人所有の太陽光・バイオマス発電によるEV充電と地域エネルギーレジリエンス（蓄電池はフィージビリティスタディで判断）、小規模なコミュニティ所有のデータセンターという3つの相互補完的な活動を、単一の調整組織のもとに統合します。
...
**ビジョン（2050年）。** 御杖村は農村再生の自立したモデルとなる。再生した在来種の森林、個人所有の太陽光・バイオマス発電によるEV充電と地域エネルギーレジリエンス、小規模なコミュニティ所有のデータセンターが一体となって、村の経済・生態系・デジタルの未来を支える。このモデルはオープンに文書化され、それを応用したいと願うすべてのコミュニティが自由に利用できる。
...
### 4.2 再生可能エネルギー・EV充電・地域レジリエンス
個人所有の太陽光発電と森林間伐材を原料とするバイオマス発電によるEV充電インフラと停電レジリエンス。出力は地域消費（村のEV充電・データセンター稼働・停電時のバックアップ電源）を想定し、経済的に適切な場合はFIT/FIPによる系統売電も行う。

バイオマス発電は山林所有者への収入と地元雇用に直結する。バイオマス発電＋太陽光＋EV充電システムは、データセンターへの24時間安定電力供給とEV普及対応を同時に実現する。現地発電設備は停電時のバックアップ電源としても機能し、農村部の老朽化した配電線に対するレジリエンスを提供する。蓄電池の設置はフィージビリティスタディで判断する。

### 4.3 コミュニティ所有のデータセンター
閉校となった御杖小学校を、地域由来の再生可能エネルギーで完全稼働する小規模・省エネのエッジコンピュータ施設として再活用する。地域産のバイオマス発電で動くコミュニティ所有のAIデータセンターを構築することで、農村コミュニティが技術的進歩と環境の持続可能性を一体として統合できることを実証する。対立するものとしてではなく、単一のシステムとして。施設の規模は、ハイパースケール経済ではなくコミュニティの説明責任に合わせて設定する。
...
| `mitsue_qa_briefing.md` | EV充電・停電レジリエンス・データセンター・太陽光・25年地平線に関するよくある質問への回答（バイリンガル） |
